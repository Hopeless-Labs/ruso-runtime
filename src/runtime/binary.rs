...
fn write_metadata(w: &mut Writer, metadata: &CheckMetadata) {
    ...
    // VERSION bump; `report_title` was likewise *removed* (between `author` and
    // `cve`) during beta without a bump, as a coordinated break — the registry
    // is redeployed and all scripts re-published, so no older `.bc` survives to
    // be misread. Outside such a coordinated reset, bump VERSION instead.
